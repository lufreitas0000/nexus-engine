...
def split_large_payload(input_path: Path, max_size_mb: float = 14.0) -> list[Path]:

    """If the artifact exceeds max_size_mb, split it sequentially to clear MTA limits."""
    ...
    # Estimate pages per part based on ratio

    parts_required = int((size_mb // max_size_mb) + 1)

    pages_per_part = total_pages // parts_required
    ...
    for part in range(parts_required):

        start = part * pages_per_part

        end = total_pages if part == parts_required - 1 else (part + 1) * pages_per_part
        ...
        part_path = input_path.with_name(f"{input_path.stem}_Part{part+1}.pdf")

        

        part_metadata = metadata.copy()

        part_metadata['/Title'] = f"{metadata['/Title']} (Part {part+1})"

        writer.add_metadata(part_metadata)
        ...
        output_artifacts.append(part_path)

        

    input_path.unlink() # Remove the oversized master file
