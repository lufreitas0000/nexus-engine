...
def main() -> None:
    ...
    parser.add_argument(
        "input_file", type=str, help="Target path to the source file (.md or .pdf)"
    )
    parser.add_argument(
        "--keep",
        action="store_true",
        help="Deprecated: artifacts are now auto-archived",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="Oasis",
        choices=list(KINDLE_MODELS.keys()),
        help="Target Kindle model for optimization.",
    )
    ...
    if not source_path.exists():
        sys.exit(
            f"Fatal: Input boundary invalid. Target file not found.\nPath evaluated: {source_path}"
        )
    ...
    try:
        ...
        if user_intent != "y":
            sys.exit(
                f"Dispatch aborted by user. Artifacts retained locally in: {artifacts_dir}"
            )
