import pytest
from orchestrator.router import route_task, LOCAL_VRAM_LIMIT_GB

def test_route_task_local(monkeypatch):
    monkeypatch.setattr("orchestrator.router.get_available_vram_gb", lambda: 6.0)
...
def test_route_task_remote_no_vram(monkeypatch):
    monkeypatch.setattr("orchestrator.router.get_available_vram_gb", lambda: 2.0)
    queue = route_task({"task_id": "test"}, 4.0)
    assert queue == "colab_queue"
